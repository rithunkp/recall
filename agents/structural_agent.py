"""Frozen DINOv2 structural embeddings and prototype-distance novelty scoring.

The Structural Agent catches frames that look visually different from familiar
prototype frames. DINOv2 stays frozen; only prototype arrays are stored.
"""
...
class NoveltyResult:
    """Structural novelty score for one frame."""
...
def load_rgb_image(path: Path) -> Image.Image:
    """Load one image as RGB for DINOv2 preprocessing."""
...
class StructuralAgent:
    """Frozen DINOv2 embedder plus cosine-distance prototype scorer."""

    def __init__(self) -> None:
        config.set_seed()
        self.processor = AutoImageProcessor.from_pretrained(config.DINO_MODEL_NAME)
        self.model = AutoModel.from_pretrained(config.DINO_MODEL_NAME).to(config.DEVICE)
        self.model.eval()
        for parameter in self.model.parameters():
            parameter.requires_grad_(False)

    def embed_images(self, image_paths: Iterable[Path]) -> np.ndarray:
        """Embed images with frozen DINOv2 CLS features."""
        paths = list(image_paths)
        embeddings: list[np.ndarray] = []
        for start in range(0, len(paths), config.STRUCTURAL_BATCH_SIZE):
            batch_paths = paths[start : start + config.STRUCTURAL_BATCH_SIZE]
            ...
            with torch.inference_mode():
                ...
                batch = outputs.last_hidden_state[:, 0].detach().cpu().numpy()
            embeddings.append(batch)
        if not embeddings:
            raise ValueError("No images were provided for structural embedding.")
    ...
    def score_embedding(self, embedding: np.ndarray, prototypes: np.ndarray) -> float:
        """Return cosine distance to the nearest structural prototype."""
...
def build_prototypes(
    ...
    max_prototypes: int = config.STRUCTURAL_MAX_PROTOTYPES,
) -> np.ndarray:
    """Build a deterministic prototype bank from train-manifest frames only."""
    if not records:
        raise ValueError("Cannot build structural prototypes from an empty manifest.")
...
def save_prototypes(
    prototypes: np.ndarray, path: Path = config.STRUCTURAL_PROTOTYPES_PATH
) -> None:
    """Persist structural prototype embeddings."""
    path.parent.mkdir(parents=True, exist_ok=True)
    np.save(path, prototypes.astype(np.float32))


def load_prototypes(path: Path = config.STRUCTURAL_PROTOTYPES_PATH) -> np.ndarray:
    """Load structural prototype embeddings."""
    return np.load(path)
...
def main() -> int:
    """Build train-only structural prototypes and smoke-score one train frame."""
    ...
    print(f"Saved structural prototypes: {config.STRUCTURAL_PROTOTYPES_PATH}")
